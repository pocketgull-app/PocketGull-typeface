"""
PocketGull Name Table Fix + All-Variant Recompiler

1. Fixes the copyright/name table on all .ttf files
2. Recompiles the 9 master SVG glyphs into every variant
3. Removes fake Greek cmap aliases
"""
import os
import hashlib
...
# Font variant configs: (filename, family, subfamily, weight_class, postscript)
VARIANTS = [
    ('PocketGull-Bold.ttf',       'PocketGull', 'Bold',    700, 'PocketGull-Bold'),
    ('PocketGull-Antigravity.ttf', 'PocketGull', 'Bold',    700, 'PocketGull-Bold'),
    ('PocketGull-Chiseltip.ttf',  'PocketGull', 'Black',   900, 'PocketGull-Chiseltip'),
    ('PocketGull-Fineliner.ttf',  'PocketGull', 'Regular', 400, 'PocketGull-Fineliner'),
    ('PocketGullMono-Regular.ttf', 'PocketGull Mono', 'Regular', 400, 'PocketGullMono-Regular'),
]

# Fake Greek aliases to remove
FAKE_CMAP_ENTRIES = [0x03B1, 0x03B2, 0x03A9]  # alpha, beta, Omega
...
def fix_name_table(font, family, subfamily, postscript):
    """Replace all name records with correct PocketGull metadata."""
    name_table = font['name']
    
    name_updates = {
        0: 'Copyright 2026 The PocketGull Project Authors (https://github.com/philgear/pocketgull-typeface)',
        1: family,
        2: subfamily,
        3: f'{postscript};2026',
        ...
        5: 'Version 2.000',
        6: postscript,
        7: 'PocketGull is a trademark of The PocketGull Project Authors.',
        8: 'The PocketGull Project Authors',
        9: 'Phil Gear',
        11: 'https://github.com/philgear/pocketgull-typeface',
        13: 'This Font Software is licensed under the SIL Open Font License, Version 1.1.',
        14: 'https://openfontlicense.org',
    }

    for nameID, value in name_updates.items():
        # Update for all platform/encoding/language combos that exist
        updated = False
        for record in name_table.names:
            if record.nameID == nameID:
                record.string = value
                updated = True
        # If no existing record, add for Windows platform
        if not updated:
            name_table.setName(value, nameID, 3, 1, 0x0409)


def remove_fake_greek(font):
    """Remove fake Greek cmap aliases (alpha->a, beta->b, Omega->O)."""
    removed = []
    for table in font['cmap'].tables:
        if hasattr(table, 'cmap'):
            for cp in FAKE_CMAP_ENTRIES:
                if cp in table.cmap:
                    del table.cmap[cp]
...
def main():
    # Pre-compute SVG bounds
    bounds_map = {}
    for char, d_path in GLYPH_MAP.items():
        bounds_map[char] = get_svg_bounds(d_path)

    for filename, family, subfamily, weight, ps_name in VARIANTS:
        ...
        # 1. Fix name table
        fix_name_table(font, family, subfamily, ps_name)
        print(f"  [OK] Name table: {family} {subfamily} ({ps_name})")

        # 2. Update version in head
        font['head'].fontRevision = 2.0

        # 3. Compile master glyphs
        glyf_table = font['glyf']
        hmtx_metrics = font['hmtx'].metrics

        for char, d_path in GLYPH_MAP.items():
            glyph, aw, lsb = compile_glyph(font, d_path, bounds_map[char])
            glyf_table[char] = glyph
            hmtx_metrics[char] = (aw, lsb)

        print(f"  [OK] Compiled {len(GLYPH_MAP)} master glyphs (origin-normalized, proportional)")

        # 4. Remove fake Greek
        removed = remove_fake_greek(font)
        if removed:
            print(f"  [OK] Removed fake cmap aliases: {', '.join(removed)}")
        else:
            print(f"  [--] No fake Greek aliases found")
