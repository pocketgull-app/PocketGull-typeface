"""
PocketGull Typeface Compiler v2 — Origin-Normalized, Proportionally-Spaced

Fixes every issue from the audit:
1. Translates each SVG path to origin (subtracts per-glyph xMin)
2. Flips Y axis from SVG (top-down) to font (bottom-up)
3. Calculates proportional advance widths from actual glyph geometry
4. Uses correct scale for 1024 UPM grid
5. Two-pass approach: first pass finds bounds, second pass draws with correct transform
"""
import os
import hashlib
...
# --- Constants ---
VIEWBOX_HEIGHT = 79.1607
BASELINE_SVG = 79.0       # Where most letter bottoms sit in SVG Y coords
ASCENDER_FONT = 800        # Target ascender height in font units
SCALE = ASCENDER_FONT / BASELINE_SVG  # ~10.127
LSB = 40                   # Left sidebearing (font units)
RSB = 40                   # Right sidebearing (font units)

INPUT_FONT = r'c:\Users\philg\Pocketgull\pocketgull-typeface\PocketGull-Bold.ttf'
OUTPUT_FONT = r'c:\Users\philg\Pocketgull\pocketgull-typeface\PocketGull-Bold.ttf'

# --- Master SVG paths mapped to their correct letter ---
# Order determined by analyze_wordmark.py bounding box X-sort
GLYPH_MAP = {
...
def get_svg_bounds(d_path):
    """Parse an SVG path and return its bounding box (xMin, yMin, xMax, yMax)."""
...
def compile_glyph(font, char, d_path, svg_bounds):
    """
    Compile a single SVG path into a properly-normalized TrueType glyph.
    
    Two-pass: bounds were pre-computed, now we draw with the correct transform.
    """
    svg_xMin, svg_yMin, svg_xMax, svg_yMax = svg_bounds
    svg_width = svg_xMax - svg_xMin

    # Transform matrix: scale, flip Y, translate to origin
    # new_x = SCALE * svg_x + dx  (where dx shifts xMin to LSB)
    # new_y = -SCALE * svg_y + dy (where dy sets baseline to y=0)
    dx = -svg_xMin * SCALE + LSB
    dy = BASELINE_SVG * SCALE  # ~800, so svg_y≈79 maps to font_y≈0
    ...
    # Proportional advance width from actual glyph geometry
...
def main():
    print("PocketGull Typeface Compiler v2")
    print(f"  Scale: {SCALE:.4f}  (ascender={ASCENDER_FONT}, baseline_svg={BASELINE_SVG})")
    ...
    # Pass 1: Compute bounding boxes
    print("Pass 1: Computing SVG bounding boxes...")
    bounds_map = {}
    for char, d_path in GLYPH_MAP.items():
        bounds = get_svg_bounds(d_path)
        bounds_map[char] = bounds
        xMin, yMin, xMax, yMax = bounds
        print(f"  '{char}': svg x=[{xMin:.2f}..{xMax:.2f}] y=[{yMin:.2f}..{yMax:.2f}]  w={xMax-xMin:.2f}")

    # Pass 2: Compile glyphs with correct transforms
    print()
    print("Pass 2: Compiling origin-normalized glyphs...")
    for char, d_path in GLYPH_MAP.items():
        glyph, aw, lsb = compile_glyph(font, char, d_path, bounds_map[char])
        glyf_table[char] = glyph
        hmtx_metrics[char] = (aw, lsb)

        bbox = f"({glyph.xMin},{glyph.yMin})-({glyph.xMax},{glyph.yMax})" if hasattr(glyph, 'xMin') else "n/a"
        nc = glyph.numberOfContours
        npts = len(glyph.coordinates) if hasattr(glyph, 'coordinates') else 0
        print(f"  '{char}': aw={aw:4d}  lsb={lsb:3d}  bbox={bbox}  {nc} contours, {npts} pts")
    ...
    # Save
    out_tmp = OUTPUT_FONT.replace('.ttf', '-v2.ttf')
    font.save(out_tmp)

    try:
        if os.path.exists(OUTPUT_FONT):
            os.chmod(OUTPUT_FONT, 0o666)
            os.remove(OUTPUT_FONT)
        os.rename(out_tmp, OUTPUT_FONT)
        final_path = OUTPUT_FONT
    except Exception:
        final_path = out_tmp
    ...
    # Quick sanity check
    print()
    print("Sanity check - re-reading compiled font:")
    vfont = TTFont(final_path)
    for char in GLYPH_MAP:
        g = vfont['glyf'][char]
        m = vfont['hmtx'].metrics[char]
        bbox = f"({g.xMin},{g.yMin})-({g.xMax},{g.yMax})" if hasattr(g, 'xMin') and g.xMin is not None else "n/a"
        print(f"  '{char}': aw={m[0]:4d}  bbox={bbox}  contours={g.numberOfContours}")
    vfont.close()
